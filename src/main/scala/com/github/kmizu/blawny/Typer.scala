...
import com.github.kmizu.blawny.AST.ClassDeclaration
...
class Typer extends Processor[AST.Program, TypedAST.Program] {
  ...
  def doTypeRecords(recordDeclarations :List[ClassDeclaration]): RecordEnvironment = {
  ...
  def typeOf(e: AST, environment: Environment = BuiltinEnvironment, records: RecordEnvironment = BuiltinRecordEnvironment, modules: ModuleEnvironment = BuiltinModuleEnvironment): Type = {
    ...
    val (typedE, s) = doType(r.doRewrite(e, Set.empty), TypeEnvironment(environment, Set.empty, records, modules, None), a, EmptySubstitution)
    s.replace(a)
  }
  ...
  def doType(e: AST, env: TypeEnvironment, t: Type, s0: Substitution): (TypedAST, Substitution) = {
    ...
    e match {
      ...
      case AST.Let(location, variable, optionalType, value, body) =>
        ...
        val newEnv = env.updateMutableVariable(variable, generalize(declaredType, env.variables))
        val (typedBody, s) = doType(body, newEnv, t, s2)
        (TypedAST.LetDeclaration(typedBody.type_, location, variable, declaredType, typedValue, typedBody), s)
