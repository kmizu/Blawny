...
import com.github.kmizu.blawny.AST.{Assignment, Block, ByteSuffix, EnumDeclaration, FloatSuffix, FunctionDefinition, Id, Import, LongSuffix, MethodDefinition, Placeholder, Program, ClassDeclaration, Selector, ShortSuffix, VarDeclaration}
import com.github.kmizu.blawny.Type.{TBoolean, TByte, TDouble, TDynamic, TFloat, TInt, TLong, TShort, TString, TUnit, TVariable}
...
class Parser extends Processor[String, Program] {
  private object KlassicParsers extends SCombinator[Program] {
    ...
    lazy val FUNCTION: Parser[String] = token("function")
    lazy val RETURN: Parser[String] = token("return")
    ...
    lazy val KEYWORDS: Set[String] = Set(
      "<", "_", ">", "<=", ">=", "+", "-", "*", "/", "{", "}", "[", "]", ":", "?",
      "if", "else", "while", "return", "foreach", "import", "cleanup", "true", "false", "in", ",", ".",
      "class", "function", "val", "clazz", "=", "==", "=>", "new", "&", "|", "&&", "||"
    )
    ...
    lazy val program: Parser[Program] = rule{
      (SPACING >> %%) ~ `import`.repeat0By(TERMINATOR) ~ clazz.repeat0By(TERMINATOR) ~ (lines << (TERMINATOR).?) << EOF ^^ {
    ...
    lazy val clazz: Parser[ClassDeclaration] = rule {
      for {
        location <- %%
        _ <- CLASS
        name <- commit(sident)
        ...
        _ <- commit(RBRACE)
      } yield ClassDeclaration(location, name, ts.getOrElse(Nil), members, methods)
    }
    ...
    lazy val linesWithReturn: Parser[Block] =
      (SPACING >> %%) ~ (line.repeat0By(TERMINATOR) << TERMINATOR.?) ~ returnExpression ^^ { case location ~ expressions ~ lastExpression =>
        Block(location, expressions ++ List(lastExpression))
      }
    ...
    //ifExpression ::= "if" expression NL "(" NL lines NL ")" "else" NL "(" NL lines NL ")"
    lazy val ifExpression: Parser[AST] = rule{
      (%% << IF) ~ commit((expression << TERMINATOR) ~ (CL(LPAREN) >> lines << CL(RPAREN)) ~ (ELSE << TERMINATOR) ~ (CL(LPAREN) >> lines << RPAREN)) ^^ {
        case location ~ (condition ~ positive ~ _ ~ negative) => IfExpression(location, condition, positive, negative)
      }
    }

    //whileExpression ::= "while" expression NL "(" NL lines NL ")"
    lazy val whileExpression: Parser[AST] = rule{
      (%% << WHILE) ~ commit((expression << TERMINATOR) ~ (CL(LPAREN) >> lines << RPAREN)) ^^ {
        case location ~ (condition ~ body) => WhileExpression(location, condition, body)
      }
    }

    //foreachExpression ::= "foreach" ident "in" expression NL lines
    lazy val foreachExpression: Parser[AST] = rule {
      (%% << FOREACH) ~ commit((ident << IN) ~ (expression << TERMINATOR) ~ (CL(LPAREN) >> lines << RPAREN)) ^^ {
    ...
    lazy val valDeclaration: Parser[VarDeclaration] = rule((%% << CL(VAL ^^ { _ => true })) ~ commit(ident ~ (typeAnnotation.? << CL(EQ)) ~ expression) ^^ {
      case location ~ (valName ~ optionalType ~ value) => VarDeclaration(location, valName.name, optionalType, value)
    })
    ...
    lazy val methodDefinition: Parser[MethodDefinition] = rule {
      (%% << CL(FUNCTION)) ~ commit(ident ~ (CL(LPAREN) >> (ident ~ typeAnnotation.?).repeat0By(CL(COMMA)) << CL(RPAREN)).? ~ (typeAnnotation.?) ~ expression) ^^ {
    ...
    lazy val returnExpression: Parser[AST] = RETURN >> expression

    // functionDefinition ::= "def" ident  ["(" [param {"," param}] ")"] NL lines returnExpression
    lazy val functionDefinition: Parser[FunctionDefinition] = rule {
      (%% << CL(FUNCTION)) ~ commit(ident ~ (CL(LPAREN) >> (ident ~ typeAnnotation.?).repeat0By(CL(COMMA)) << CL(RPAREN)).? ~ (typeAnnotation.?) ~ linesWithReturn) ^^ {
        case location ~ (functionName ~ params ~ optionalType ~ body) =>
          val ps = params match {
            case Some(xs) =>
              xs.map {
