...
import com.github.kmizu.blawny.Type.TVariable
...
object AST {
  ...
  case class Program(location: Location, imports: List[Import], records: List[ClassDeclaration], block: Block)

  case class Import(location: Location, simpleName: String, fqcn: String)

  case class ClassDeclaration(location: Location, name: String, ts: List[TVariable], members: List[(String, Type)], methods: List[MethodDefinition])
  ...
  case class VarDeclaration(location: Location, variable: String, type_ : Option[Type], value: AST) extends AST
  ...
  case class Let(location: Location, variable: String, type_ : Option[Type], value: AST, body: AST) extends AST
  object Let {
    def apply(variable: String, type_ :Option[Type], value: AST, body: AST): Let = {
      Let(NoLocation, variable, type_, value, body)
    }
  }
  ...
  case class FunctionDefinition(location: Location, name: String, body: Lambda) extends AST

  case class MethodDefinition(location: Location, name: String, body: Lambda) extends AST

  case class LetRec(location: Location, name: String, function: Lambda, body: AST) extends AST
  object LetRec {
